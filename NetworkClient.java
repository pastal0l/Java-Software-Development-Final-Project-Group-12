...
class NetworkClient {
    ...
    // -----------------------------------------------------------------------
    volatile int    myPlayerId = -1;
    volatile int    levelIdx   = 0;

    // Map data (set during setup, then immutable)
    volatile int[][]      serverMap;
    volatile int          mapSize;
    volatile List<double[]> serverBalls = new ArrayList<>();

    // -----------------------------------------------------------------------
    // Game state (written by reader thread, read on game thread)
    // -----------------------------------------------------------------------
    volatile double  remotePlayerX;
    volatile double  remotePlayerY;
    volatile double  remotePlayerAngle;
    ...
    volatile double[]  monsterY       = new double[0];
    volatile boolean[] monsterChasing = new boolean[0];
     ...
     */
    final Queue<int[]> diamondsTaken = new ConcurrentLinkedQueue<>();
